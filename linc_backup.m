...
% Author: E.Matus
% 6/30/2017
% Last edit: 7/10/17 SD

%--------------------%
% Configuration      %
%--------------------%
t.cfg.mode       = 1;               % outphasing mode for a(t)=A(t)cos(wc.t+phi(t)) : 
                                    %                   1:theta=arcsin(abs(A(t)); 
                                    %                   2;theta=arccos(abs(A(t)) 
                                    %                   3;theta1=arcsin(real(a(t)),theta2=arcsin(imag(a(t))
t.cfg.src        = 8;               % 1:BPSK, 2:harmonic, 3:LP-noise, 4:Rectangle, 5:Ramp, 6:Duo-harmonic, 7:OFDM, 8.Kronecker
t.cfg.fs         = 20*10^6;         % IQ sampling frequency
t.cfg.L          = 64;              % IQ vector length
t.cfg.cp         = 50;              % cyclic prefix
t.cfg.sps        = 4;               % IQ - oversampling
t.cfg.aFiltFlag  = 0;               % use combiner transfer function
t.cfg.lpFiltFlag = 0;               % Spectrum shaping filter
t.cfg.errType    = 2;               % 1:comb-rrcFilt, 2:irrcFilt-iq; 
t.cfg.rngInit    = 1;               % 1: init random genetator to produce sama sequence 0: don't init                      
...
t.iq.cp          = t.cfg.cp;                         % cyclic prefix: total length=L+CP 

t.iq.Wm          = floor(0.8 * t.iq.L/2)/t.iq.L;     % modulation freq. of sin wave
t.iq.modType     = 2;                                % QAM type: 1-QAM, 2-16QAM, 3-64QAM, 4-256QAM
t.iq.numSc       = fix(0.8 * t.iq.L/2) * 2;          % number of data subcarriers in OFDM symbol
c = fix(t.iq.numSc/2);
t.iq.idxSc       =  [2:c+1, t.iq.L-c+1:t.iq.L];  
t.iq.n1          = 1;                                % Data limits including     
t.iq.n2          = t.iq.n1 + t.iq.L - 1;             % Data limits including                    

if t.cfg.src  == 1,
    t.iq.do = reshape([ones(1,floor(t.iq.L/2)); -ones(1,floor(t.iq.L/2))], [], 1)';
elseif t.cfg.src == 2,
    t.iq.do = exp(j*2*pi*t.iq.Wm *(0:t.iq.L-1));
elseif t.cfg.src == 3,
    order = 80;
    B = fir1(order,[0.85]);                         % white noise coloring filter
    t.iq.do = filter(B,1,randn(1, t.iq.L+t.iq.cp)); 
    t.iq.do = complex(t.iq.do, filter(B,1,randn(1, t.iq.L+t.iq.cp))); 
    t.iq.do = t.iq.do(t.iq.cp:end);
elseif t.cfg.src == 4,   
    L2 = floor(t.iq.L/2);
    t.iq.do = [zeros(1,L2), ones(1,t.iq.L-L2)];
elseif t.cfg.src == 5,   
    t.iq.do = 0:(t.iq.L-1);
elseif t.cfg.src == 6,
    t.iq.do = exp(j*2*pi*t.iq.Wm *(0:t.iq.L-1)) + 0.7*exp(j*2*pi*t.iq.Wm*0.5 *(0:t.iq.L-1));
elseif t.cfg.src == 7,
    numSym = 2^(2*t.iq.modType);
    refSym = qammod(0:(numSym-1),numSym);
    s = modnorm(refSym,'peakpow',1);
    x = randi([0 numSym-1], 1 ,t.iq.numSc);
    y = s*qammod(x,numSym);
    z = zeros(1,t.iq.L);
    z(t.iq.idxSc) = y;
    t.iq.di = z;
    % scatterplot(t.iq.di);
    t.iq.do = ifft(t.iq.di);
elseif t.cfg.src == 8,   
    t.iq.do = zeros(1,t.iq.L);
    t.iq.do(1) = 1;
end
t.iq.do = [t.iq.do(end-t.iq.cp+1:end) t.iq.do];     % add CP
%t.iq.do = t.iq.do - sum(t.iq.do)/t.iq.L;           % remove DC
t.iq.do = 0.9 * t.iq.do/max(abs(t.iq.do));          % scale amplitude |iq|<0.9
...
elseif t.cfg.mode == 2
    ...
    t.pm.do{2} =  0.5*complex( +cos(t.angle.beta ),  +sin(t.angle.beta ));
elseif t.cfg.mode == 3
